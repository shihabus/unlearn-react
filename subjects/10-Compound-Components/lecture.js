...
function TabList(props) {
  const { _onSelection, _activeIndex } = props;
  const children = React.Children.map(props.children, (child, idx) => {
    const _isActive = idx === _activeIndex;
    return React.cloneElement(child, {
      _onSelection: () => _onSelection(idx),
      _isActive,
    });
  });
  return <div>{children}</div>;
...
function Tab(props) {
  return (
    <div
      onClick={props.disabled ? null : props._onSelection}
      style={props._isActive ? styles.activeTab : styles.tab}
    >
      {props.children}
    </div>
  );
      ...
      <div>
        <Tabs2>
          ...
          <TabList>
            <Tab>Tacos</Tab>
            <Tab>Burritos</Tab>
            <Tab disabled>Coconut Korma</Tab>
          </TabList>
        </Tabs2>
      </div>
    );
